...
for name in uci_datasets_info:
    ...
    for pocket_size in pocket_sizes:
        if os.path.isfile(f'./perceptive_graphs/{name}_{pocket_size}.pkl'):
            ...
            if os.path.isfile(f'./results/ablation/{name}_p{pocket_size}.pkl'):
                continue

            print(f"{name}_p{pocket_size}")
            slurm = open(f"{name}_p{pocket_size}_ablation.slurm", "w")
            slurm.write(
f"""#!/bin/bash
...
# The name of the job:
#SBATCH --job-name="{name}_p{pocket_size}_ablation"
...
"""
            ...
            os.system(f"sbatch {name}_p{pocket_size}_ablation.slurm")